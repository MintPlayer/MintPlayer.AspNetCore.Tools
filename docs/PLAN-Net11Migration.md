...
## M0 — Spikes (before any implementation)

- [ ] **S1 — Does an empty multi-targeted project pack the analyzer twice?**
...
- [ ] **S2 — Are the `ASPDEPR004`/`ASPDEPR008` APIs deprecated or removed on `net11.0`?**
...
- [ ] **S3 — Does the Endpoints generator leak a framework assembly to consumers?**
...
- [ ] **S4 — Does the `GetTargetPath` cross-project call survive an outer/inner build?**
...
- [ ] **S5 — Can `Microsoft.AspNetCore.Mvc.Testing` be TFM-conditioned cleanly?**
...
- [ ] **S6 — Which `rollForward` policy survives RC → GA without a second edit?**
...
- [ ] S1 has a reproduced error (or a clean pack) and a chosen fix.
- [ ] S2 has a compile result, and M3's size is known.
- [ ] S3 has a verified answer on both TFMs, and R3.4's cost is known.
- [ ] S5 confirms a warning-free TFM-conditioned reference.
- [ ] S6 has a chosen `rollForward`.
